...
bool DuPlanner::find_lowest_cost_trajectory(
  ...
  while (true) {
    ...
    if (result == ruckig::Result::ErrorInvalidInput) {
      std::cerr << "MIDI: Invalid input to trajectory generator" << std::endl;
      continue;
    }
    if (result != ruckig::Result::Working) {
      std::cerr << "MIDI: Trajectory generation error" << std::endl;
      continue;
    }
    ...
    // CPU-only collision checking (MIDI method)
    ...
    for (uint8_t i = 0; i < segments.size(); i++) {
      if (const auto* second_order_segment = dynamic_cast<const common_math::SecondOrderSegment*>(segments[i])) {
        ...
        // In MIDI method, we primarily use second order segments
        continue;
      } else {
        is_collision_free = false;
        break;
...
// CPU-only collision checking for MIDI method
bool DuPlanner::is_segment2_collision_free(const SecondOrderSegment* original_segment, double& trajectory_collision_probability, double& mahalanobis_distance) {
  const PinholeCamera& camera = _camera;
  
  // Skip if the segment is too short
  if (original_segment->get_duration() <= 1e-6) return true;
  
  // Get start and end points
  Eigen::Vector3d start_point = original_segment->get_start_point();
  Eigen::Vector3d end_point = original_segment->get_end_point();
  
  // Skip if the segment is closer than the vehicle radius
  if (end_point.z() <= camera.get_true_vehicle_radius()) return true;

  // For CPU version, we simplify: just use the whole segment if start is valid
  // In CUDA version, they split at vehicle radius, but for CPU we can skip that complexity
  double start_checking_time = original_segment->get_start_time();
  if (start_point.z() < camera.get_true_vehicle_radius()) {
    // If start is too close, skip a bit of the trajectory (simplified approach)
    start_checking_time = original_segment->get_start_time() + 0.01;  // Skip 10ms
