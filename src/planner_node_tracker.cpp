...
void PlannerNode::update_reference_trajectory() {
  ...
  if (trajectory_queue_.empty()) {
    ...
    // Only re-arm in TRAJECTORY_CONTROL / WAITING_FOR_OPUS — HOLDING_WAYPOINT
    // and ALIGNING_HEADING have no goal-direction depth image yet (img_callback
    // is gated out), and after Goal reached / Brake / Land the drone's CANCEL
    // already removed it from the coordinator's trajectory database.
